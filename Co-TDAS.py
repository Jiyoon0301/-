...
class Qnet(nn.Module):
    ...
    def sample_action(self, s, epsilon, battery,rate):
        out = self.forward(s)
        q_list ,idx = [], []
        for i in range(0,21,1):
            if (0<= battery + i <= battery_max) and (-battery_rate <= rate + i <= battery_rate):
                q_list.append(out[i].item())
                idx.append(i)
            else: q_list.append(np.Inf)
...
def next_state(n,n_epi,time,battery,charge,day_charge,TOU,load_data,generation_data):
...
def cal_price(n,time, charge, day_charge,TOU):
    cost = charge * TOUs[n][time]
    if time == 23: cost = cost + DCs[n] * max(day_charge)
    return cost
...
for n_epi in tqdm(range(epochs)):
    ...
    for n in range(N_new):
        ...
        for time in range(24):
            ...
            cost = cal_price(n,time,action,actions,TOU)
            costs += cost
            state_prime = next_state(n,n_epi,time,battery,action,actions,TOUs[n],load_data,generation_data)
